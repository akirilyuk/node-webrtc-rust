...
- **speech** — Default spoken language ID identifies at `user_speaking_end` when buffered speech reaches `minSpeechMs`, not mid-utterance at that threshold. Hang-up LID defers until TTS is queued or drains (short grace after `user_speech_final` so echo handlers can call `sendTextToTTS` first). `user_speech_final` now precedes `user_speaking_end` on the STT final path. Continuous mode (`languageId.continuous: true`) still identifies mid-utterance when TTS is idle.
