...
- **examples (sherpa):** echo-LID roundtrip harness reply prefix is now `Okay.` — Kroko emits no token for ~5% of Piper `echo.` renders even with full context (audio verified intact), which made the 10-session `roundtrip-counting-echo-lid-multi` gate fail by construction; the STT-level prefix assertion is unchanged.
- **speech:** STT pre-roll flush left-pads with silence to the ring capacity when a new recognizer stream starts from a cold inbound path (no frames before the talker's onset), so Zipformer always gets lead-in context before the first spoken word; Sherpa `roundtrip-counting-echo-lid` no longer drops the reply prefix when the listener had a silent inbound track.
...
Recover VoiceAgent TTS playback after drain writer failures and the 45s job-playback timeout so a later TTS job can synthesize again.
...
Align all publishable packages on `0.6.5` and ship root connection-error handling (`helpers@0.6.4` is already on npm).
...
- **All publishable packages** — version `0.6.5`; internal pins aligned so nested installs (e.g. via `@node-webrtc-rust/helpers`) resolve the same SDK/signaling build.
...
SDK fix: advanced voice VAD timeout overrides now reach the native pipeline.
