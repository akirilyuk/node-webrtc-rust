...
| **voice-agent-local-sherpa**                                           | Browser + Node server   | 3002             | Sherpa browser demo + CI roundtrip E2E scripts — [`ROUNDTRIP.md`](./voice-agent-local-sherpa/ROUNDTRIP.md) (incl. `roundtrip-language-id`, `roundtrip-barge-in` / `roundtrip-barge-in-buffered`, `roundtrip-tts-stream`, counting/echo/…)                                      |
| **voice-agent-local-sherpa** `start:roundtrip-language-id`             | CLI (exits on success)  | —                | Multi-language TTS legs → `user_language` ISO 639-1 (needs `download-lid` + per-lang TTS) — [ROUNDTRIP.md](./voice-agent-local-sherpa/ROUNDTRIP.md#run-modes)                                                                                                                  |
| **voice-agent-local-sherpa** `start:roundtrip-counting-echo-lid-multi` | CLI (exits on success)  | —                | **10 concurrent** echo+LID sessions (ultimate `maxActiveConnectionsPerPod`) via `VoiceAgentSessionHost`, ultimate pool env, full one…ten + LID ordering — [ROUNDTRIP.md](./voice-agent-local-sherpa/ROUNDTRIP.md#counting-echo--lid--10-concurrent-sessions-ultimate-pool-env) |
| **voice-agent-local-sherpa-multi-client**                              | Browser + Node (3 tabs) | 3004             | **Three clients, one room**; bundled WAV via `playAudio` on connect; `start:mix-groups` for Voice+Data positional mix — [README](./voice-agent-local-sherpa-multi-client/README.md)                                                                                            |
...
| voice-agent-local-sherpa              | `download-stt:en` + `download-tts:en`; `NWR_RECORD_FORMAT=wav npm run start:record --workspace=@node-webrtc-rust/example-voice-agent-local-sherpa` (listen-back WAV) or `bash scripts/ci/run-sherpa-example-ci.sh e2e`    | Sherpa E2E scripts in CI Test job — [ROUNDTRIP.md](./voice-agent-local-sherpa/ROUNDTRIP.md#ci-github-actions)                                   |
| voice-agent-local-sherpa language-id  | `npm run download-lid --workspace=@node-webrtc-rust/example-voice-agent-local-sherpa` + `download-tts` for en/de/es; `npm run start:roundtrip-language-id --workspace=@node-webrtc-rust/example-voice-agent-local-sherpa` | Prints `user_language` per leg (en/de/es); exits 0 when all ISO codes match — [ROUNDTRIP.md](./voice-agent-local-sherpa/ROUNDTRIP.md#run-modes) |
...
| `EADDRINUSE` on example ports      | Re-run `npm run start`. **Multi-client:** npm `prestart` runs [`scripts/free-port.sh`](../scripts/free-port.sh) before Node starts; most browser examples also call [`shared/free-port.ts`](./shared/free-port.ts) at bind time. Set `VOICE_SKIP_FREE_PORT=1` to disable both. Manual: `PORT=3004 bash scripts/free-port.sh` — [scripts/README.md](../scripts/README.md#free-portsh) |
