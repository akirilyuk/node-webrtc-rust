...
Local STT (no cloud): [`voice-agent-local-sherpa`](../voice-agent-local-sherpa/README.md) — **free on-device Sherpa STT** (privacy-friendly: mic audio stays off third-party STT APIs; lower latency); per-language `download-model:es|fr|de|zh|…`; full list in [`VOICE_VENDOR_REFERENCE`](../shared/VOICE_VENDOR_REFERENCE.md#local-sherpa-onnx--free-on-device-stt-recommended) · [Sherpa-ONNX docs](https://k2-fsa.github.io/sherpa/onnx/)
