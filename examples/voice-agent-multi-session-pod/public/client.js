/**
 * Browser client for the multi-session voice pod demo.
 *
 * Flow:
 * 1. POST /api/sessions — pod joins the session room and prepares a VoiceAgentSessionHost
 * 2. WebSocket join — browser enters the same signaling room
 * 3. Server offer — pod creates RTCPeerConnection + VoiceAgent for this connection
 * 4. Disconnect — VoiceAgent.stop() + PC close; pod tears down idle session slot
 */
...
function startMicVisualizer(stream) {
  ...
  if (micVizStatusEl) {
    micVizStatusEl.textContent = 'Live — mic audio sent to this session’s VoiceAgent'
  }
}
...
async function connect() {
  ...
  sessionId = sessionInput.value.trim() || 'call-1'
  clientId = `client-${Math.random().toString(36).slice(2, 10)}`

  setStatus(`Allocating session "${sessionId}" on pod…`)
  connectButton.disabled = true

  const ensureResponse = await fetch('/api/sessions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ sessionId }),
  })
  if (!ensureResponse.ok) {
    setStatus('Failed to allocate session on pod')
  ...
  ws.onopen = () => {
    sendSignal({ type: 'join', room: sessionId, peerId: clientId })
    setStatus(`Session "${sessionId}" joined — waiting for VoiceAgent offer…`)
...
function disconnect() {
  ...
  setStatus('Disconnected — pod should tear down idle session')
}
  ...
  switch (message.type) {
    case 'peer-joined':
      if (message.peerId === SERVER_PEER_ID) {
        appendEvent('client', 'Pod VoiceAgent host joined session')
      }
      break
    case 'offer':
      if (message.peerId === SERVER_PEER_ID) {
...
async function onServerOffer(sdp) {
  ...
  serverPc.onconnectionstatechange = () => {
    if (serverPc?.connectionState === 'connected') {
      setStatus(`WebRTC connected — session ${sessionId}`)
