/**
 * Guards SDP so we never use an offer/answer that webrtc-rs will reject with
 * `set_remote_description called with no ice-ufrag`.
 *
 * Lives in sdk (not signaling) so the CI sdk-core bootstrap build can validate
 * local descriptions before `@node-webrtc-rust/signaling` dist exists.
 */
