...
build_linux_target() {
  ...
  if [[ "$target" == "x86_64-unknown-linux-gnu" ]]; then
    (cd "$BINDINGS" && npx napi build --platform --release --features otel,rnnoise --target "$target")
  else
    (cd "$BINDINGS" && npx napi build --platform --release --features otel,rnnoise --target "$target" --zig)
  fi
...
build_linux_musl_target() {
  ...
  docker run --rm \
    ...
    "$alpine_image" \
    bash -lc 'npx napi build --platform --release --features otel,rnnoise --target '"$target"
  docker run --rm \
    -e SHERPA_ONNX_LIB_DIR=/opt/sherpa-musl/lib \
    -e LD_LIBRARY_PATH=/opt/sherpa-musl/lib:/usr/lib \
    -v "$ROOT:/workspace" \
